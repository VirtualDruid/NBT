...
public class Chunk implements Iterable<Section> {
    ...
    private void initReferences(long loadFlags) {
        ...
        //
        if (level == null) {
            throw new IllegalArgumentException("data older than 1.18 does not contain \"Level\" tag");
        }
        ...
        if ((loadFlags & ENTITIES) != 0) {
            entities = level.containsKey("Entities") ?
                    level.getListTag("Entities").asCompoundTagList() :
                    null;

                    //1.18 entities at different file
        }
        if ((loadFlags & TILE_ENTITIES) != 0) {
            tileEntities = level.containsKey("TileEntities") ?
                    level.getListTag("TileEntities").asCompoundTagList() :

                    //1.18
                    level.containsKey("block_entities") ?
                            level.getListTag("block_entities").asCompoundTagList() :
                            null;
        }
        if ((loadFlags & TILE_TICKS) != 0) {
            tileTicks = level.containsKey("TileTicks") ?
                    level.getListTag("TileTicks").asCompoundTagList() :

                    //1.18
                    level.containsKey("block_ticks") ?
                            level.getListTag("block_ticks").asCompoundTagList() :
                            null;
        }
        if ((loadFlags & LIQUID_TICKS) != 0) {
            liquidTicks = level.containsKey("LiquidTicks") ?
                    level.getListTag("LiquidTicks").asCompoundTagList() :

                    //1.18
                    level.containsKey("fluid_ticks") ?
                            level.getListTag("fluid_ticks").asCompoundTagList() :
                            null;
        }
        if ((loadFlags & LIGHTS) != 0) {
            lights = level.containsKey("Lights") ?
                    level.getListTag("Lights").asListTagList() :
                    null;
        }
        if ((loadFlags & LIQUIDS_TO_BE_TICKED) != 0) {
            liquidsToBeTicked = level.containsKey("LiquidsToBeTicked") ?
                    level.getListTag("LiquidsToBeTicked").asListTagList() :
                    null;
        }
        if ((loadFlags & TO_BE_TICKED) != 0) {
            toBeTicked = level.containsKey("ToBeTicked") ?
                    level.getListTag("ToBeTicked").asListTagList() :
                    null;
        }
        if ((loadFlags & POST_PROCESSING) != 0) {
            postProcessing = level.containsKey("PostProcessing") ?
        ...
        if ((loadFlags & STRUCTURES) != 0) {
            ...
            structures = structures == null ?
                    level.getCompoundTag("structures") : structures;
        }

        //fix for 1.18
        if ((loadFlags & (BLOCK_LIGHTS | BLOCK_STATES | SKY_LIGHT)) != 0) {

            ListTag<CompoundTag> sectionTagList;
            try {
                boolean lesserThanVersion18 = level.containsKey("Sections");
                boolean isHigherVersion18 = level.containsKey("sections");
                sectionTagList = lesserThanVersion18 ?
                        level.getListTag("Sections").asCompoundTagList() :
                        null;
                sectionTagList = sectionTagList == null && isHigherVersion18 ?
                        level.getListTag("sections").asCompoundTagList() :
                        null;
                if (lesserThanVersion18 && isHigherVersion18) {
                    throw new IllegalArgumentException("conflict version of chunk format between older version and 1.18");
                }
                if (sectionTagList == null) {
                    throw new IllegalArgumentException("data dose not contains \"Sections\"/\"sections\" tag");
                }
            } catch (NullPointerException e) {
                throw new IllegalStateException();
            }

            for (CompoundTag section : sectionTagList) {
